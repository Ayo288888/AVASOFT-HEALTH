...
Removes all caps/limits to process 100% of all raw source files.
"""
...
def build_all_canonical():
    print("--- Building Canonical JSONL Datasets from Raw Sources (Full Uncapped) ---")
    
    # 1. DDXPlus (Full 1.02M+ patient dataset)
    ...
    print(f"Processing DDXPlus from {ddx_path} (Uncapped)...")
    c1 = process_ddxplus_csv(str(ddx_path), "data/canonical/ddxplus.jsonl")
    print(f"DDXPlus Canonical Records: {c1:,}")
    
    # 2. Kaggle 773 (Full 246k+ disease-symptom dataset)
    ...
    print(f"Processing Kaggle 773 from {kag_path} (Uncapped)...")
    c2 = process_kaggle773_csv(str(kag_path), "data/canonical/kaggle773.jsonl")
    print(f"Kaggle 773 Canonical Records: {c2:,}")
    
    # 3. AfriMed-QA v2 (Full 15.2k+ Q&A clinical dataset)
